...
1. **Архитектура проекта** ✅
   ...
   - [x] Определить стратегию code splitting для маршрутов

2. **Документация архитектурных решений (ADR)** ✅
   - [x] Создать шаблон для ADR документов
   - [x] Задокументировать ключевые архитектурные решения:
     - [x] Выбор технологического стека
     - [x] Стратегия state management (Zustand vs Redux Toolkit)
     - [x] Подход к оффлайн-синхронизации
     - [x] Структура данных в Firestore
     - [x] Стратегия code splitting
     - [x] Feature-based структура папок
   - [x] Настроить процесс обновления ADR при изменении архитектуры

3. **Настройка окружения** ✅
   - [x] Инициализировать проект: `npm create vite@latest finpal -- --template react-ts`
   - [x] Настроить ESLint + Prettier + Husky для code quality
   - [x] Конфигурировать TypeScript с strict режимом
   - [x] Настроить Zod для валидации и генерации типов
...
5. **Базовая конфигурация Firebase** ✅
   - [x] Создать Firebase проект (конфигурация готова, требуется создание проекта в Firebase Console)
   - [x] Инициализировать Firebase CLI (конфигурационные файлы созданы)
...
- [x] Firebase проект создан и подключен
- [x] Созданы ADR документы для ключевых архитектурных решений
...
1. **Настройка Firebase сервисов** ✅
   - [x] Активировать Firestore, Authentication, Hosting
   - [x] Настроить Firestore с оффлайн-персистентностью
   - [x] Создать базовые Security Rules для аутентификации

2. **Система аутентификации** ✅
   - [x] Реализовать вход через Google (Firebase Auth)
   - [x] Создать HOC/хук `useAuth` для защиты роутов
   - [x] Страница: Login
   - [ ] Страница: Register
   - [ ] Страница: Forgot Password

3. **State Management (Zustand)**
   - [x] Создать store для пользователя (`useAuthStore`)
   - [ ] Создать store для UI состояния (`useUIStore`)
   - [x] Настроить персистентность для оффлайн-доступа (auth store)
...
- [x] Firestore сохраняет данные локально при оффлайн
